...
function initApp() {
  ...
  console.log(
    "%cA.R.L.O. - AssembleJS Repository Logic Orchestrator",
    "font-size: 14px; font-weight: bold; color: #2196F3;"
  );
}

// Initialize task-related handlers
function initTaskHandlers() {
  // Watch for view changes to handle task UI initialization
  ARLO.utils.eventBus.on("view-changed", (data) => {
    if (data.view === "workflow" || data.view === "task") {
      // Initialize task controls if they exist
      if (typeof window.setupTaskControls === 'function') {
        window.setupTaskControls();
      }
      
      // Try to resume any active task connections
      const currentTaskId = window.currentTaskId;
      if (currentTaskId && typeof window.connectToTaskUpdates === 'function') {
        // Check if task is in an active state before connecting
        const taskElement = document.querySelector(`.task-item[data-task-id="${currentTaskId}"]`);
        if (taskElement) {
          const statusElement = taskElement.querySelector('.task-meta');
          if (statusElement && 
              (statusElement.textContent.includes('analyzing') || 
               statusElement.textContent.includes('planning') || 
               statusElement.textContent.includes('executing') || 
               statusElement.textContent.includes('validating'))) {
            window.connectToTaskUpdates(currentTaskId);
          }
        }
      }
    }
...
function initSidebar() {
  ...
  const sidebarCollapsed = ARLO.utils.storage.get(
    "arlo_sidebar_collapsed",
    false
  );
  if (sidebarCollapsed) {
