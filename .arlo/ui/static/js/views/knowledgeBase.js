...
function initKnowledgeBaseView() {
    ...
    const contentContainer = knowledgeBaseView.querySelector('.kb-content') || 
                            knowledgeBaseView.querySelector('.content-body');
    
    if (!contentContainer) return;
    
    // Creating from scratch each time to ensure we don't have duplicates
    contentContainer.innerHTML = `
        <div class="kb-tabs">
            <div class="kb-tab active" data-view="search">Search</div>
            <div class="kb-tab" data-view="network">Knowledge Network</div>
            <div class="kb-tab" data-view="collections">Collections</div>
        </div>
        
        <div class="kb-layout">
            <!-- Search View -->
            <div id="kb-search-tab-view" class="kb-tab-content active">
                <div class="kb-sidebar">
                    <div class="kb-search-container">
                        <div class="kb-search-box">
                            ...
                            <button id="kb-search-button" class="kb-search-button">
                        ...
                        <div class="kb-search-filters">
                            <div class="kb-search-filter">
                                <label for="kb-search-agent">Agent</label>
                                <select id="kb-search-agent" class="kb-search-agent">
                            ...
                            <div class="kb-search-filter">
                                <label for="kb-search-type">Type</label>
                                <select id="kb-search-type" class="kb-search-type">
                                    <option value="">All Types</option>
                                    <option value="task">Tasks</option>
                                    <option value="task_knowledge">Knowledge</option>
                                    <option value="task_completion">Completions</option>
                                    <option value="file_upload">Uploads</option>
                                </select>
                            </div>
                            <div class="kb-search-filter">
                                <label for="kb-search-limit">Results</label>
                                <select id="kb-search-limit" class="kb-search-limit">
                    ...
                    <div id="kb-search-results" class="kb-search-results">
                        <div class="empty-state">
                            ...
                            <div class="empty-state-text">Enter a search query to find knowledge</div>
                        </div>
                    </div>
                </div>
                <div class="kb-main">
                    <div id="kb-search-view" class="kb-view active">
                        <div class="empty-state">
                            <div class="empty-state-icon">📚</div>
                            <div class="empty-state-text">Select a result from the sidebar to view details</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Network View -->
            <div id="kb-network-tab-view" class="kb-tab-content">
                <div class="kb-network-controls">
                    <div class="kb-network-filter">
                        <label for="network-filter">Filter Connections:</label>
                        <select id="network-filter">
                            ...
                            <option value="weak">Weak Connections</option>
                        </select>
                    </div>
                    <div class="kb-network-layout">
                    ...
                    <div class="kb-network-focus">
                        <label for="network-focus">Focus Node:</label>
                        <select id="network-focus">
                ...
                <div class="kb-network-visualization">
                    <div class="kb-network-container" id="kb-network-graph">
                        <!-- Network graph will be rendered here -->
                        <div class="kb-network-placeholder">
                            <div class="kb-node-container">
                                <!-- Central node -->
                                <div class="kb-central-node" style="left: 50%; top: 50%">
                                    <span>Knowledge</span>
                                </div>
                                
                                <!-- Agent nodes in a circle around the center -->
                                <div class="kb-agent-node" style="left: 30%; top: 30%; background-color: var(--color-admin);">
                                    <span>Admin</span>
                                </div>
                                <div class="kb-agent-node" style="left: 70%; top: 30%; background-color: var(--color-browser);">
                                    <span>Browser</span>
                                </div>
                                <div class="kb-agent-node" style="left: 70%; top: 70%; background-color: var(--color-server);">
                                    <span>Server</span>
                                </div>
                                <div class="kb-agent-node" style="left: 30%; top: 70%; background-color: var(--color-utils);">
                                    <span>Utils</span>
                                </div>
                                
                                <!-- Knowledge nodes -->
                                <div class="kb-knowledge-node" style="left: 40%; top: 40%;">
                                    <span>Components</span>
                                </div>
                                <div class="kb-knowledge-node" style="left: 60%; top: 40%;">
                                    <span>Events</span>
                                </div>
                                <div class="kb-knowledge-node" style="left: 60%; top: 60%;">
                                    <span>Renderers</span>
                                </div>
                                <div class="kb-knowledge-node" style="left: 40%; top: 60%;">
                                    <span>Types</span>
                                </div>
                            </div>
                            
                            <!-- Edges are represented with SVG lines -->
                            <svg class="kb-connections" width="100%" height="100%">
                    ...
                    <div class="kb-network-details" id="kb-network-details">
                        <div class="empty-state">
                            <div class="empty-state-icon">🔍</div>
            ...
            <!-- Collections View -->
            <div id="kb-collections-tab-view" class="kb-tab-content">
                ...
                <div id="kb-collections-view" class="kb-collections-content">
    ...
    tabButtons.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            knowledgeBaseView.querySelectorAll('.kb-tab').forEach(t => t.classList.remove('active'));
            knowledgeBaseView.querySelectorAll('.kb-tab-content').forEach(c => c.classList.remove('active'));
            ...
            if (viewName) {
                const contentView = knowledgeBaseView.querySelector(`#kb-${viewName}-tab-view`);
                if (contentView) {
    ...
    const filterInputs = knowledgeBaseView.querySelectorAll('.kb-search-filter select');
    ...
    const networkControls = knowledgeBaseView.querySelectorAll('.kb-network-controls select');
...
function initKnowledgeNetwork() {
    ...
    const allNodes = document.querySelectorAll('.kb-central-node, .kb-agent-node, .kb-knowledge-node');
...
function createConnection(svg, sourceNode, targetNode, type) {
    ...
    line.setAttribute('stroke', type === 'primary' ? 'rgba(255,255,255,0.6)' : 
                              type === 'secondary' ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.2)');
...
function updateKnowledgeNetwork() {
    ...
    connections.forEach(line => {
        if (filterType === 'direct') {
            ...
            line.style.opacity = line.getAttribute('stroke-width') === '2' ? '1' : '0.1';
        } else if (filterType === 'strong') {
            // Show primary and secondary connections
            line.style.opacity = line.getAttribute('stroke') !== 'rgba(255,255,255,0.2)' ? '1' : '0.1';
...
function showNetworkNodeDetails(node) {
    const nodeType = node.classList.contains('kb-agent-node') ? 'agent' :
                    node.classList.contains('kb-knowledge-node') ? 'knowledge' : 'central';
...
async function performKnowledgeSearch() {
    ...
    try {
        ...
        if (agentName) {
            ...
            const agents = [
                'Admin', 'Browser', 'Types', 'Utils', 'Validator', 'Developer',
                'Server', 'Testbed', 'Pipeline', 'Generator', 'Config', 'Docs',
                'Git', 'Analyzer', 'Bundler', 'Version', 'ARLO'
            ];
            
            // Query each agent's knowledge and combine results
            const allResultsPromises = agents.map(agent =>
                VectorDB.queryAgentKnowledge(agent, query, Math.ceil(limit / agents.length))
        ...
        if (docType) {
            results = results.filter(item => 
                item.metadata && item.metadata.type === docType
            );
        }
...
function updateSearchResults(results) {
    ...
    resultsContainer.innerHTML = results.map(result => {
        ...
        const date = metadata && metadata.timestamp 
        ...
        const previewText = document.length > 120 
            ? document.substring(0, 120) + '...'
            : document;
        
        return `
            <div class="kb-search-result" data-id="${metadata?.id || ''}" data-agent="${resultAgent}">
                ...
                <div class="kb-result-meta">
                    ...
                    <div class="kb-result-score">Relevance: ${Math.round(score * 100)}%</div>
                </div>
            </div>
        `;
    }).join('');
...
async function loadCollections() {
    try {
        ...
        const collectionsView = knowledgeBaseView.querySelector('#kb-collections-view');
        if (!collectionsView) return;
        
        // Calculate stats
        const totalDocuments = collections.reduce((sum, collection) => sum + (collection.count || 0), 0);
        const agentCollections = collections.filter(c => c.name.startsWith('agent_'));
        const nonAgentCollections = collections.filter(c => !c.name.startsWith('agent_'));
        ...
        collectionsView.innerHTML = `
            ...
            <div class="collections-grid">
                ${collections.map(collection => {
                    const isAgentCollection = collection.name.startsWith('agent_');
                    const agentName = isAgentCollection ? collection.name.replace('agent_', '') : null;
                    ...
                    return `
                        <div class="collection-card ${isAgentCollection ? 'agent-collection' : ''}" 
                             data-collection="${collection.name}" 
                             ${isAgentCollection ? `style="border-top: 3px solid ${agentColor};"` : ''}>
                            ...
                            <div class="collection-card-actions">
                                <button class="view-collection-btn" data-collection="${collection.name}">View</button>
                                <button class="delete-collection-btn" data-collection="${collection.name}">Delete</button>
                            </div>
                        </div>
                    `;
                }).join('')}
        ...
        const viewButtons = collectionsView.querySelectorAll('.view-collection-btn');
        ...
        const deleteButtons = collectionsView.querySelectorAll('.delete-collection-btn');
...
function showCreateCollectionModal() {
    ...
    const collectionTypeRadios = modal.querySelectorAll('input[name="collection-type"]');
    ...
    createBtn.addEventListener('click', async () => {
        ...
        const collectionType = modal.querySelector('input[name="collection-type"]:checked').value;
