...
function initializePipeline(taskId, tasks, currentTaskId) {
    ...
    task.logs.push(`[${new Date().toLocaleTimeString()}] Admin agent analyzing task...`);
...
function updatePipeline(taskId, tasks, currentTaskId) {
    ...
    const currentStep = pipelineSteps.findIndex(step => step.status === 'pending');
    
    if (currentStep >= 0) {
        // Simulate step completion after a delay
        setTimeout(async () => {
            pipelineSteps[currentStep].status = 'success';
            pipelineSteps[currentStep].time = `${Math.floor(Math.random() * 60) + 10}s`;
            task.logs.push(`[${new Date().toLocaleTimeString()}] Completed step: ${pipelineSteps[currentStep].name}`);
            ...
            if (currentAgent) {
                try {
                    ...
                    task.logs.push(`[${new Date().toLocaleTimeString()}] ${currentAgent} agent added knowledge to its collection`);
                } catch (knowledgeError) {
                    console.error(`Error adding ${currentAgent} agent knowledge:`, knowledgeError);
                    task.logs.push(`[${new Date().toLocaleTimeString()}] Error adding ${currentAgent} agent knowledge: ${knowledgeError.message}`);
                }
            }
            ...
            if (currentStep < pipelineSteps.length - 1) {
                ...
                task.logs.push(`[${new Date().toLocaleTimeString()}] Starting step: ${pipelineSteps[currentStep + 1].name}`);
                renderPipeline();
                ...
                // Pipeline complete
                task.status = 'success';
                task.response = 'Task completed successfully! PR created: https://github.com/example/repo/pull/123';
                task.logs.push(`[${new Date().toLocaleTimeString()}] Pipeline completed successfully`);
                ...
                try {
                    ...
                    task.logs.push(`[${new Date().toLocaleTimeString()}] Git agent recorded task completion`);
                } catch (error) {
...
function renderPipeline() {
    ...
    pipelineContainer.innerHTML = pipelineSteps.map(step => `
        <div class="pipeline-step">
            <div class="pipeline-step-header">
                <div class="pipeline-step-icon task-status-${step.status}"></div>
                ${step.name}
            </div>
            <div class="pipeline-step-body">
                <div class="pipeline-step-avatar" style="background-color: ${step.color};">
                    ${step.agent.charAt(0)}
                </div>
                ...
                <div class="pipeline-step-time">
                    ${step.status === 'pending' ? '<div class="spinner"></div> Running...' : step.time}
                </div>
            </div>
        </div>
    `).join('');
...
function calculatePipelineProgress(task) {
    ...
    const completedSteps = pipelineSteps.filter(step => step.status === 'success').length;
    const pendingSteps = pipelineSteps.filter(step => step.status === 'pending').length;
    
    return (completedSteps + (pendingSteps * 0.5)) / pipelineSteps.length * 100;
}

export { initializePipeline, updatePipeline, renderPipeline, pipelineSteps, calculatePipelineProgress };