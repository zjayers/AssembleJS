...
const addDocument = async (req, res) => {
  try {
    ...
    console.error(`Error adding document to collection ${req.params.name}:`, error);
    res.status(500).json({
      success: false,
...
const addDocumentBatch = async (req, res) => {
  try {
    ...
    console.error(`Error adding document batch to collection ${req.params.name}:`, error);
    res.status(500).json({
      success: false,
...
const queryCollection = async (req, res) => {
  try {
    ...
    const results = await fileDB.queryCollection(name, { query, limit, filters });
    res.json(results);
  } catch (error) {
...
const deleteDocument = async (req, res) => {
  try {
    ...
    console.error(`Error deleting document ${req.params.docId} from collection ${req.params.name}:`, error);
    res.status(500).json({
      success: false,
