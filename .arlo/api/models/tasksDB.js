...
const createTask = async (taskData) => {
  ...
  const task = {
    id: taskId,
    title: taskData.title || taskData.description.substring(0, 60) + (taskData.description.length > 60 ? '...' : ''),
    description: taskData.description,
    status: taskData.status || 'pending',
    timestamp: new Date().toISOString(),
    response: taskData.response || '',
    logs: taskData.logs || [`[${new Date().toLocaleTimeString()}] Task created: ${taskData.description}`]
  };
