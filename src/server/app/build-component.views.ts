...
export async function buildComponentViews(userOpts: BlueprintServerOptions) {
  ...
  if (
    !userOpts.manifest.components ||
    userOpts.manifest.components.length === 0
  ) {
    return [];
  }

  return Promise.all(
    userOpts.manifest.components.map(async (component) => {
      ...
      if (!checkFileExists(baseDirPath)) {
        console.warn(
          `Warning: Component root path ${rootPath} does not exist at ${baseDirPath}`
        );
      }
      ...
      if (!checkFileExists(componentViewDir)) {
        console.warn(
          `Warning: Component directory ${component.path} not found in ${rootPath}`
        );
      ...
      return {
        ...
        views: await Promise.all(
          component.views.map(async (view) => {
            ...
            if (!checkFileExists(viewDir)) {
              console.warn(
                `Warning: View directory ${view.viewName} not found in ${componentViewDir}`
              );
            ...
            if (useFileLoader && !checkFileExists(viewTemplateFilePath)) {
              console.warn(
                `Warning: Template file ${view.templateFile} not found in ${viewDir}`
              );
              ...
              return {
                ...
                getTemplate: () =>
                  `<div>Template file "${view.templateFile}" not found</div>`,
                root: component.path,
                manifestUrl: `/${component.path}/${view.viewName}/manifest/`,
                contentUrl: `/${component.path}/${view.viewName}/`,
                dataUrl: `/${component.path}/${view.viewName}/data/`,
